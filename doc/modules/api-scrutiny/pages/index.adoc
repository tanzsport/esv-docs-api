...
NOTE: Die Version 2.0.0 ist noch nicht final.
...
* Veranstaltung: Korrektur der Schnittstellenbeschreibung Feld
„bemerkung“
* Startliste: Startbuch immer gefüllt ab Schnittstellen-Level 2 auch bei
...
* Klarstellung, das JS-Score für alle Varianten des Judging Systems
inkl. AJS gilt
* Ergebnis: Kreuzvorgabe im Rundenablauf
* Ergebnis: Klarstellung, welche Startgruppe für den Starter angegeben
werden soll
...
* Arrays für Ablauf von normalen Runden [ablauf] und Stichrunden
[stichrunden] (NULL für Upload von nur-Startlisten)
** Runde mit Schlüsselwerten 1, 1A, 1B, R, 2, 3, 4, 5, 6, 7, 8, 9, F,
FA, FB (Werte bis hier nur im Array [ablauf]), PSR, RSR (diese Werte nur
im Array [stichrunden])
...
das Ergebnis z.B. bei Relegation = RSR
** Wertungstyp [typ]
...
* Startgruppe (nur Einzelwettbewerbe Std/Lat und Gruppen- bzw.
Formationswettbewerbse; sonst null; gem. Schlüsselverzeichnis)
[startgruppe] – ab Level 2
** Hier muss der ursprüngliche in der Startliste verzeichnete Wert
...
im kombinierten Hgr/Hgr II-Turnier also Jug, nicht Hgr)
* Startklasse (nur Einzelwettbewerbe Std/Lat; sonst null; gem.
Schlüsselverzeichnis) [startklasse] – ab Level 2
** Hier muss die zum Zeitpunkt des Turniers gültige Startklasse des
Starters angegeben werden.
* Wettbewerbsart (nur Einzelwettbewerbe JMD; sonst null; gem.
Schlüsselverzeichnis) [wettbewerbsart] – ab Level 2
** Hier muss die ursprüngliche in der Startliste verzeichnete
Wettbewerbsart angegeben werden.
...
[laufzettel]
** Verwarnung/Disqualifikation (0 = keine, 1 = Verwarnt
...
* Wertungen für Runden aus Rundenablauf (Array mit N = Anzahl Runden
...
anzugeben; Stichrundenwertungen werden hier nicht angegeben!) [wertung]
...
* Stichrundenwertung (nur eine möglich pro Starter, ansonsten NULL)
[stichrundenwertung] – ab Level 2
** 0-basierter Index der Stichrunde aus dem Stichrunden-Ablauf
[stichrunde]
** erhaltene Wertungen [wertung]; Struktur wie Endrundenwertung in den
...
inneres Array für den einzigen Tanz der Stichrunde
...
* Array Stichrundentabellen (ab Schnittstellen-Level 2; Reihenfolge aus Rundenablauf;
wenn keine Stichrunden durchgeführt wurden NULL) [stichrundentabellen]
...
Formationswettbewerben ohne Ergebnisrelevanz nur für Relegation = RSR)
